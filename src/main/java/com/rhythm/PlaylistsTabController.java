...
public class PlaylistsTabController extends UserEndInitializerController{
    //Variables
    ...
    @FXML
    private ListView<Playlist> _playlistsListView;
    @FXML
    private ListView<Song> _songsListView;
    @FXML
    private Label _messageLabel, _songNameLabel, _artistNameLabel, _playlistNameLabel;
    @FXML
    ...
    private void Play(){
        ...
        if (_musicPlayer != null){
            ...
            _messageLabel.setText("No Songs Selected. Create New Playlist or Add a Song To An Existing One");
        }
    }
    ...
    private void SelectPlaylist(){
        //local variable
        int _tempIndexHolder = _playlistsListView.getSelectionModel().getSelectedIndex();
        //Checks if the selected song is already selected
        if (_selectedPlaylist != _tempIndexHolder){
    ...
    private void PopulatePlaylistsListView() {
        ...
        _playlistsListView.getSelectionModel().selectFirst();
    ...
    private void SetOnVolumeChange() {
        _volumeSlider.setOnMouseDragged(mouseEvent -> {
            if (_musicPlayer != null) {
                _musicPlayer.AdjustVolume(_volumeSlider.getValue() / 100);
            }
        });

        _volumeSlider.setOnMouseReleased(mouseEvent -> {
            if (_musicPlayer != null) {
                _musicPlayer.AdjustVolume(_volumeSlider.getValue() / 100);
            }
        });
    ...
    @FXML
    private void LoadAddPlaylist() {
        Stop();
        SceneManager.LoadPopup(PopupType.AddPlaylist, _sessionID);
    }

    @FXML
    private void LoadAddSong() {
        Stop();
        SceneManager.LoadPopup(PopupType.AddSong, _sessionID);
    }

    @FXML
    private void RemoveSelectedSong() {
        Stop();
        if (_selectedPlaylist != -1 && _selectedSong != -1){
            _queryHandler.RemoveSongFromPlaylist(_playlistsListView.getSelectionModel().getSelectedItem().getPlaylistID(),
                    _songsListView.getSelectionModel().getSelectedItem().getSongID());
            RefreshTab();
        } else {
            _messageLabel.setText("No Song Selected Song To Remove");
    ...
    private void DeleteSelectedPlaylist() {
        ...
        if (_selectedPlaylist != -1){
            _queryHandler.DeletePlaylist(_playlistsListView.getSelectionModel().getSelectedItem().getPlaylistID());
            RefreshTab();
        } else {
            _messageLabel.setText("No Selected Playlist To Remove");
